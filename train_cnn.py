...
ElInelNone_train = np.zeros((n_training, 3), dtype=np.int32)
...
for dirname in subdirs:
    ...
    for fnameX in filesX:
        ...
        ElInelNone_train[ntot:ntot+n] = dataY[:,[0, 1, 2]]
        ntot += n
...
ElInelNone_test = np.zeros((n_testing, 3), dtype=np.int32)
...
for dirname in subdirs:
    ...
    for fnameX in filesX:
        ...
        ElInelNone_test[ntot:ntot+n] = dataY[:,[0, 1, 2]]
        ntot += n
print ntot, 'events ready'
...
h = model.fit({'main_input': X_train},
              {'el_inel_none_netout': ElInelNone_train},
              validation_data=(
                  {'main_input': X_test},
                  {'el_inel_none_netout': ElInelNone_test}),
              batch_size=batch_size, epochs=nb_epoch, shuffle=True,
              verbose=1)

X_train = None
ElInelNone_train = None
